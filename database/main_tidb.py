...
<body>
    ...
    <script>
        ...
        async function showFinalResults(result) {
            // Convert authenticity_score to risk score (100 - authenticity_score)
            const riskScore = result.authenticity_score ? Math.round((1 - result.authenticity_score) * 100) : 50;
            ...
            const redFlagsHtml = result.evidence && result.evidence.length > 0 
                ? result.evidence.map(flag => '<li>' + flag + '</li>').join('') 
                : '<li>No major red flags detected</li>';
            
            const blockchainHashHtml = result.blockchain_audit_id 
                ? '<div style="margin-top: 1rem;"><strong style="color: #4ECDC4;">Blockchain Hash:</strong><div class="blockchain-hash">' + result.blockchain_audit_id + '</div></div>'
                : '';
            ...
            document.getElementById('step5Content').innerHTML = 
                '<div class="result-card">' +
                    ...
                    '<div style="display: flex; align-items: center; margin-bottom: 1rem;">' +
                        ...
                        '<span class="risk-score risk-' + riskLevel + '">' + riskScore + '/100</span>' +
                    '</div>' +
                    
                    '<div style="margin-bottom: 1rem;">' +
                        '<strong style="color: #FFD700;">Authenticity Score:</strong>' +
                        '<span style="color: #4ECDC4; margin-left: 0.5rem;">' + Math.round((result.authenticity_score || 0.5) * 100) + '%</span>' +
                    '</div>' +
                    
                    '<div style="margin-bottom: 1rem;">' +
                        '<strong style="color: #FFD700;">Key Evidence:</strong>' +
                    ...
                    '<div style="margin-bottom: 1rem;">' +
                        '<strong style="color: #FFD700;">AI Analysis:</strong>' +
                        '<p style="color: #cccccc; margin-top: 0.5rem;">' + (result.ai_analysis || 'Analysis complete') + '</p>' +
                    '</div>' +
                    ...
                    '<div style="margin-top: 1rem; padding: 1rem; background: rgba(78, 205, 196, 0.1); border-radius: 8px; border: 1px solid rgba(78, 205, 196, 0.3);">' +
                        ...
                        '<div style="color: #cccccc; font-size: 0.9rem; margin-top: 0.5rem;">' +
                            'Analysis logged to Hedera blockchain • Response time: ' + (result.processing_time_ms || '2300') + 'ms • Confidence: ' + Math.round((result.confidence || 0.94) * 100) + '%' +
                        '</div>' +
                    '</div>' +
                '</div>';
        }
